...
import { Link } from 'react-router-dom';
import { Plus, Search, Filter, Trash2, Package } from 'lucide-react';
import { Product } from '../../types';
...
export const Products: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('http://localhost:9000/api/products/products');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:9000/api/products/products/${id}`, {
        method: 'DELETE',
      });
