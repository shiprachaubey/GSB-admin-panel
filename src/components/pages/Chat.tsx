...
const [pendingMessage, setPendingMessage] = useState('');
  ...
  useEffect(() => {
    axios
      .get('http://localhost:9000/api/chat/chats')
      .then((res) => {
  ...
  const handleSelectConversation = async (conversation: ChatConversation) => {
    try {
      const res = await axios.get(`http://localhost:9000/api/chat/${conversation._id}`);
      setSelectedConversation(conversation);
      setMessages(res.data.messages || []);
    } catch (err) {
      console.error('Failed to load messages:', err);
...
const handleSendMessage = async () => {
  ...
  try {
    await axios.post(`http://localhost:9000/api/chat/${selectedConversation._id}/reply`, {
       text: newMessage,
...
const handleAssignAgent = async () => {
  try {
    await axios.put('http://localhost:9000/api/chat/chats/assign', {
